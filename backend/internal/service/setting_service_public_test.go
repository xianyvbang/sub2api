...
func TestSettingService_GetPublicSettings_ExposesModelMarketplaceFlags(t *testing.T) {
	repo := &settingPublicRepoStub{
		values: map[string]string{
			SettingKeyModelMarketplaceEnabled:       "true",
			SettingKeyModelMarketplaceRequiresLogin: "false",
		},
	}
	svc := NewSettingService(repo, &config.Config{})
	...
	require.False(t, settings.ModelMarketplaceRequiresLogin)
}
