...
const apiKeyAuthSnapshotVersion = 14 // v14: include group video pricing fields and user-specific group rate cache invalidation fix
...
func (s *APIKeyService) snapshotFromAPIKey(ctx context.Context, apiKey *APIKey) *APIKeyAuthSnapshot {
	...
	snapshot := &APIKeyAuthSnapshot{
		...
		RateProtectionEnabled:   apiKey.RateProtectionEnabled,
		MaxRateMultiplier:       apiKey.MaxRateMultiplier,
		UserGroupRateMultiplier: apiKey.UserGroupRateMultiplier,
...
func (s *APIKeyService) snapshotToAPIKey(key string, snapshot *APIKeyAuthSnapshot) *APIKey {
	...
	apiKey := &APIKey{
		...
		RateProtectionEnabled:   snapshot.RateProtectionEnabled,
		MaxRateMultiplier:       snapshot.MaxRateMultiplier,
		UserGroupRateMultiplier: snapshot.UserGroupRateMultiplier,
		User: &User{
			ID:                         snapshot.User.ID,
