...
func TestSettingHandler_GetPublicSettings_ExposesModelMarketplaceFlags(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	require.NoError(t, json.Unmarshal(recorder.Body.Bytes(), &resp))
	require.Equal(t, 0, resp.Code)
	require.True(t, resp.Data.ModelMarketplaceEnabled)
	require.False(t, resp.Data.ModelMarketplaceRequiresLogin)
}
