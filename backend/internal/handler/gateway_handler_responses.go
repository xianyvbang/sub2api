...
func (h *GatewayHandler) Responses(c *gin.Context) {
	...
	// /v1/responses remains token-priced even when it also declares image tools.
	requestCtx, pricingAt := service.WithGatewayTokenRequestPricing(requestCtx)
	imageIntent := service.IsImageGenerationIntentForPlatform("/v1/responses", reqModel, body, openAICompatibleRequestPlatform(requestCtx, apiKey))
